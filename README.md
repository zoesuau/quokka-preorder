...
- 商品可使用總庫存，或啟用款式個別庫存與代表照片；單款為 0 只停售該款，全部售罄才自動下架
- 訂單取消會在同一個 Script Lock 內回補有限庫存；款式商品只回補原款式，重複取消不會再次回補
...
- 手機商品後台：拍照／相簿上傳、建立、編輯、款式個別照片與庫存，並可從商品卡片調整一般商品庫存與上下架
...
- `Products`：商品名稱、分類、最多 10 張圖片、台幣售價、款式、庫存、狀態與排序；`variantOptionsJson` 保存款式識別碼、個別庫存與代表照片。舊商品未啟用時仍沿用總庫存。
...
- `node tests/variant-inventory.test.js`：驗證款式資料、個別扣庫存、最後一件防超賣、取消回補、舊商品相容與前後台介面契約。
