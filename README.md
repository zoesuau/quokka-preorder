...
- `Products`：商品名稱、分類、最多 10 張圖片、台幣售價、款式、庫存、狀態與選填的置頂順序。
...
- `Settings`：前台說明、分類順序、共用計價、訂金／付款規則、停賣公告和 iOPEN Mall 設定。前台先依人工分類順序排列；同分類內置頂商品優先，其餘依台灣繁中筆畫排列。舊匯款欄位只保留備份，不再顯示或提供前台使用。
